...
import com.cmt.statemachine.util.AssertUtil;
...
public class StateMachineImpl<S, E> implements StateMachine<S, E> {
    ...
    @Override
    public <R> S fireEvent(S sourceStateId, E event, R request) {
        isReady();
    ...
    @Override
    public void generateStateDiagram(int width, int height) {
        AssertUtil.notPositive(width, "width must be positive");
        ...
        try {
            if (height <= 0) {
                Graphviz.fromGraph(graph).width(width).render(Format.PNG).toFile(new File("StateDiagram"));
            } else {
                Graphviz.fromGraph(graph).width(width).height(height).render(Format.PNG).toFile(new File("StateDiagram"));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
